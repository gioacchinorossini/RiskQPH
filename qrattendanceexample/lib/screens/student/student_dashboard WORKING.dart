...
class _StudentDashboardState extends State<StudentDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    // Get screen width for horizontal centering calculations
    final double screenWidth = MediaQuery.of(context).size.width;

    // Calculate dynamic QR code size based on scroll offset
    final double qrSize = 200 - (_scrollOffset * 0.3).clamp(0, 80);

    // Calculate dynamic top position for the QR code
    final double qrTopPosition = 100 - (_scrollOffset * 0.2).clamp(0, 60);

    // Calculate name opacity
    final double nameOpacity = (1 - (_scrollOffset / 100)).clamp(0, 1);

    // Calculate year level opacity
    final double yearLevelOpacity = (1 - (_scrollOffset / 120)).clamp(0, 1);
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          // Animated Year Level
          Positioned(
            top: qrTopPosition + qrSize + 60,
            left: 0,
            right: 0,
            child: Opacity(
              opacity: yearLevelOpacity,
              child: Center(
                child: Text(
