...
class _StudentDashboardState extends State<StudentDashboard> {
  ...
  bool _lockEnabled = true; // Control whether lock feature is enabled
  Offset _dragButtonPosition = const Offset(20, 100); // Initial position of draggable button
      ...
      _scrollThrottleTimer = Timer(const Duration(milliseconds: 16), () {
        if (mounted) {
          setState(() {
            ...
            // Check if SliverAppBar has been collapsed (when scroll offset exceeds expanded height - collapsed height)
            // Only lock if lock feature is enabled
            if (_lockEnabled) {
              final double expandedHeight = MediaQuery.of(context).size.height * 1.0;
              final double collapsedHeight = MediaQuery.of(context).size.height * 0.15;
              final double collapseThreshold = expandedHeight - collapsedHeight;
  ...
  Widget build(BuildContext context) {
    ...
    final bool isSFDUO = screenWidth > 500;
    final bool issamsungs8plus = screenWidth < 361;
    final bool laptop = screenWidth > 700;
    ...
    if (isTablet) {
      ...
      maxQrSize = (sliverAppBarHeight * 0.40).clamp(200, 350); // 40% of SliverAppBar height, between 200-350px
    } else if (isSFDUO) {
      ...
      // On laptop, use 30% for better foldable proportions
      maxQrSize = (sliverAppBarHeight * 0.10).clamp(50, 200); // 30% of SliverAppBar height, between 180-300px
    } else {
    ...
    // If SliverAppBar has been collapsed and lock is enabled, use the minimum collapsed size (same as original clamp values)
    if (_hasBeenCollapsed && _lockEnabled) {
      if (isTablet) {
        qrSize = maxQrSize - (1000 * 0.4).clamp(0, maxQrSize * 0.5); // Same as original clamp minimum
      } else if (isSFDUO) {
        qrSize = maxQrSize - (1000 * 0.35).clamp(0, maxQrSize * 0.45); // Same as original clamp minimum
      } else if (issamsungs8plus) {
        qrSize = maxQrSize - (1000 * 0.3).clamp(0, maxQrSize * 0.4); // Same as original clamp minimum
      } else if (laptop) {
        qrSize = maxQrSize - (1000 * 0.25).clamp(0, maxQrSize * 0.3); // Same as original clamp minimum
      } else {
        qrSize = maxQrSize - (1000 * 0.3).clamp(0, maxQrSize * 0.6); // Same as original clamp minimum
      }
    } else {
      // Normal shrinking behavior before collapse
      if (isTablet) {
        // Tablets: Shrink by 40% of scroll, minimum 50% of original size
        qrSize = maxQrSize - (_scrollOffset * 0.4).clamp(0, maxQrSize * 0.5);
      } else if (isSFDUO) {
        // Samsung Fold Duo: Shrink by 35% of scroll, minimum 45% of original size
        qrSize = maxQrSize - (_scrollOffset * 0.35).clamp(0, maxQrSize * 0.45);
      } else if (issamsungs8plus) {
        // Samsung S8 Plus: Shrink by 30% of scroll, minimum 40% of original size
        qrSize = maxQrSize - (_scrollOffset * 0.3).clamp(0, maxQrSize * 0.4);
      } else if (laptop) {
        // Laptops: Shrink by 25% of scroll, minimum 30% of original size
        qrSize = maxQrSize - (_scrollOffset * 0.25).clamp(0, maxQrSize * 0.3);
      } else {
        // Regular phones: Shrink by 30% of scroll, minimum 60% of original size
        qrSize = maxQrSize - (_scrollOffset * 0.3).clamp(0, maxQrSize * 0.6);
      }
    }
    ...
    if (_hasBeenCollapsed && _lockEnabled) {
      // If collapsed and locked, use the final collapsed position
      qrTopPosition = collapsedHeight * 0.1; // 10% from top of collapsed SliverAppBar
    } else {
      // Normal movement before collapse or when lock is disabled
      qrTopPosition = startingTopPosition - (_scrollOffset * 0.5).clamp(0, maxUpwardMovement);
    }
    ...
    if (_hasBeenCollapsed && _lockEnabled) {
      // If collapsed and locked, use the final left position
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          RefreshIndicator(
            ...
            child: CustomScrollView(
              ...
              slivers: [
                ...
                SliverAppBar(
                  expandedHeight: (_hasBeenCollapsed && _lockEnabled)
                ...
                SliverToBoxAdapter(
                  child: Container(
                    height: qrSize * 1, // Space for QR code + welcome section + extra padding
          ...
          if (_scrollOffset <= 200 && !(_hasBeenCollapsed && _lockEnabled)) ...[
            Positioned(
          ...
          Positioned(
            ...
            child: AnimatedContainer(
              ...
              height: qrSize,
              width: (_scrollOffset > 200 || (_hasBeenCollapsed && _lockEnabled)) ? screenWidth - (isTablet ? 160 : 80) : qrSize, // Expand to full width minus equal spacing (more spacing on tablets)
              ...
              child: (_scrollOffset > 200 || (_hasBeenCollapsed && _lockEnabled)) 
                ? Row(
                    children: [
                      // QR Code Widget
                      SizedBox(
              ...
              children: [
                            Text(
                              ...
                              style: TextStyle(
                                ...
                                fontSize: isTablet ? 20 : 16, // Larger text on tablets
                                fontWeight: FontWeight.bold,
                              ),
              ...
              child: AnimatedOpacity(
                ...
                child: Center(
                  child: Text(
                    ...
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: isTablet ? 36 : 28, // Larger text on tablets
                      fontWeight: FontWeight.bold,
                    ),
                    textAlign: TextAlign.center,
                  ),
          ...
          if (_scrollOffset < 10 && !(_hasBeenCollapsed && _lockEnabled)) ...[
            Positioned(
          ...
          // Unlock Arrow Button (only visible when collapsed and locked)
          if (_hasBeenCollapsed && _lockEnabled) ...[
            Positioned(
          ...
          // Draggable Lock Control Button
          Positioned(
            left: _dragButtonPosition.dx,
            top: _dragButtonPosition.dy,
            child: GestureDetector(
              onPanUpdate: (details) {
                setState(() {
                  _dragButtonPosition = Offset(
                    _dragButtonPosition.dx + details.delta.dx,
                    _dragButtonPosition.dy + details.delta.dy,
                  );
                });
              },
              child: Material(
                ...
                child: InkWell(
                  onTap: () {
                    setState(() {
                      _lockEnabled = !_lockEnabled;
                      if (!_lockEnabled) {
                        // If disabling lock, also reset the collapsed state
                        _hasBeenCollapsed = false;
                      }
                    });
                  },
                  ...
                  child: Container(
                    ...
                    height: 50,
                    decoration: BoxDecoration(
                      color: _lockEnabled ? AppTheme.primaryColor : Colors.green,
                      shape: BoxShape.circle,
                      boxShadow: [
                        BoxShadow(
