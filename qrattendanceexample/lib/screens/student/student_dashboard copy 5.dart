...
class _StudentDashboardState extends State<StudentDashboard> {
      ...
      _scrollThrottleTimer = Timer(const Duration(milliseconds: 16), () {
        if (mounted) {
          setState(() {
            ...
            // Reset collapsed state when user scrolls back to top (within SliverAppBar area)
            if (_scrollOffset <= collapsedHeight && _hasBeenCollapsed) {
  ...
  Widget build(BuildContext context) {
    ...
    // Calculate SliverAppBar collapsed height (moved up for QR size calculation)
    final double collapsedHeight = MediaQuery.of(context).size.height * 0.15;
    
    // Calculate dynamic QR code size based on scroll offset with media query support
    double qrSize;
    
    // If SliverAppBar has been collapsed and we're in content area, use the minimum collapsed size
    if (_hasBeenCollapsed && _scrollOffset > collapsedHeight) {
      if (isTablet) {
    ...
    if (_hasBeenCollapsed && _scrollOffset > collapsedHeight) {
      // If collapsed and in content area, use the final collapsed position
      qrTopPosition = collapsedHeight * 0.1; // 10% from top of collapsed SliverAppBar
    } else {
      // Normal movement before collapse or when in SliverAppBar area
    ...
    if (_hasBeenCollapsed && _scrollOffset > collapsedHeight) {
      // If collapsed and in content area, use the final left position
        ...
        children: [
          RefreshIndicator(
            ...
            child: CustomScrollView(
              ...
              slivers: [
                // SliverAppBar with collapsible main section - original behavior within SliverAppBar area
                SliverAppBar(
                  expandedHeight: MediaQuery.of(context).size.height * 1.0, // Always allow full expansion
          ...
          if (_scrollOffset <= 200 && !(_hasBeenCollapsed && _scrollOffset > collapsedHeight)) ...[
            Positioned(
          ...
          Positioned(
            ...
            child: AnimatedContainer(
              ...
              height: qrSize,
              width: (_scrollOffset > 200 || (_hasBeenCollapsed && _scrollOffset > collapsedHeight)) ? screenWidth - (isTablet ? 160 : 80) : qrSize, // Expand to full width minus equal spacing (more spacing on tablets)
              ...
              child: (_scrollOffset > 200 || (_hasBeenCollapsed && _scrollOffset > collapsedHeight)) 
                ? Row(
                    children: [
                      // QR Code Widget
                      SizedBox(
          ...
          if (_scrollOffset < 10 && !(_hasBeenCollapsed && _scrollOffset > collapsedHeight)) ...[
            Positioned(
          ...
          // Unlock Arrow Button (only visible when collapsed and in content area)
          if (_hasBeenCollapsed && _scrollOffset > collapsedHeight) ...[
            Positioned(
