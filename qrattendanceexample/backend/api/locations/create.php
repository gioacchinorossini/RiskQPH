...
try {
    $stmt = $db->prepare('INSERT INTO locations (name, description, is_active, created_at, updated_at) VALUES (?, ?, 1, NOW(), NOW())');
    $stmt->execute([$name, $description]);
    $id = (int)$db->lastInsertId();
} catch (Throwable $e) {
    respond(500, ['success' => false, 'message' => 'Failed to create location']);
}
